...
FileLocker::FileLocker (int fd, unsigned start, unsigned length)
: itsFD          (fd),
  itsError       (0),
  itsStart       (start),
  itsLength      (length),
...
bool FileLocker::acquire (LockType type, unsigned nattempts)
{
    itsError = 0;
...
///	      ' '<<itsStart<<' '<<itsLength<<endl;
    ...
    for (unsigned i=0; i<nattempts; i++) {
	if (fcntl (itsFD, F_SETLK, &ls) != -1) {
	    itsError = 0;
	    itsReadLocked = true;
	    if (type == Write) {
...
bool FileLocker::canLock (unsigned& pid, LockType type)
{
#if defined(AIPS_NOFILELOCK)
    return true;
#else
...
std::string FileLocker::lastMessage() const
{
    if (itsError == 0) {
	return "";
    }
